...
public class ConversationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        secondaryThumbnailVideoView = (VideoView) findViewById(R.id.secondary_thumbnail_video_view);
    ...
    private void addParticipant(Participant participant) {
		...
		if (participantIdentity == "") {
			...
			if (participant.getVideoTracks().size() > 0) {
				addParticipantVideo(participant, participant.getVideoTracks().get(0));
			}		
			...
			//videoStatusTextView.setText("Participant "+ participantIdentity + " joined as main");
		}
		else if( secondaryParticipantIdentity == "" ){
			/*
			 * Add secondary participant renderer
			 */
			if (participant.getVideoTracks().size() > 0) {
				addSecondaryParticipantVideo(participant.getVideoTracks().get(0));
			}				
			/*
			 * Start listening for participant events
			 */
			...
			secondaryParticipantIdentity = participant.getIdentity();
			//videoStatusTextView.setText("Participant "+ secondaryParticipantIdentity + " joined as secondary");
		}
		else {
			Snackbar.make(connectActionFab, "More than 3 participants are not currently support in this UI", Snackbar.LENGTH_LONG).setAction("Action", null).show();
		}
    }
    ...
    private void addParticipantVideo( Participant participant, VideoTrack videoTrack) {
		...
		else if(participant.getIdentity() == secondaryParticipantIdentity){
			addSecondaryParticipantVideo(videoTrack);
		}
    }
	
	private void addSecondaryParticipantVideo(VideoTrack videoTrack) {
        if (secondaryThumbnailVideoView.getVisibility() == View.GONE) {
            secondaryThumbnailVideoView.setVisibility(View.VISIBLE);
		}
		secondaryThumbnailVideoView.setMirror(false);
        secondaryThumbnailVideoView.setVideoScaleType(VideoScaleType.ASPECT_BALANCED);
        videoTrack.addRenderer(secondaryThumbnailVideoView);
    }

    private void moveLocalVideoToThumbnailView() {
        if (thumbnailVideoView.getVisibility() == View.GONE) {
            thumbnailVideoView.setVisibility(View.VISIBLE);
            thumbnailVideoView.setVideoScaleType(VideoScaleType.ASPECT_BALANCED);
	...
	private void moveSecThumbnailViewToLocalView(VideoTrack videoTrack) {
        if (secondaryThumbnailVideoView.getVisibility() == View.VISIBLE) {
            secondaryThumbnailVideoView.setVisibility(View.GONE);
            secondaryThumbnailVideoView.setVideoScaleType(VideoScaleType.ASPECT_BALANCED);
            videoTrack.removeRenderer(secondaryThumbnailVideoView);
            videoTrack.addRenderer(primaryVideoView);
        }
    }
	
	private void showfeedbackMSG(String MSG){
		Snackbar.make(connectActionFab, MSG, Snackbar.LENGTH_LONG).setAction("Action", null).show();
	}
    ...
    private void removeParticipant(Participant participant) {
        ...
        if (participant.getIdentity().equals(participantIdentity)) {
            if (participant.getVideoTracks().size() > 0) {
				removeParticipantVideo(participant, participant.getVideoTracks().get(0));
			}
			...
			if(secondaryParticipant == null){
				...
				moveSecThumbnailViewToLocalView(secondaryParticipant.getVideoTracks().get(0));
				participantIdentity = secondaryParticipantIdentity;
				mainParticipant = secondaryParticipant;
				secondaryParticipantIdentity = "";
				secondaryParticipant = null;
			}			
        } 
		else if (participant.getIdentity().equals(secondaryParticipantIdentity)) {
            if (participant.getVideoTracks().size() > 0) {
				removeSecondaryParticipantVideo(participant.getVideoTracks().get(0));
			}
			participant.setListener(null);
			moveSecondaryView();
			secondaryParticipantIdentity = "";
	...
	private void moveSecondaryView() {
        if (secondaryThumbnailVideoView.getVisibility() == View.VISIBLE) {
            secondaryParticipant.getVideoTracks().get(0).removeRenderer(secondaryThumbnailVideoView);
            secondaryThumbnailVideoView.setVideoScaleType(VideoScaleType.ASPECT_BALANCED);
            secondaryThumbnailVideoView.setVisibility(View.GONE);
        }
	}
    ...
    private Room.Listener roomListener() {
        return new Room.Listener() {
            ...
            public void onConnected(Room room) {
				...
				localParticipantIdentity = localParticipant.getIdentity();
				//videoStatusTextView.setText("Connected to " + room.getName());
                //videoStatusTextView.setText("Waiting on other participant to join");
                setTitle(room.getName());

                for (Participant participant : room.getParticipants()) {
                    if(localParticipantIdentity != participant.getIdentity()){
            ...
            public void onConnectFailure(Room room, TwilioException e) {
                videoStatusTextView.setText("Falha na conexão");
            ...
            public void onParticipantDisconnected(Room room, Participant participant) {
                //videoStatusTextView.setText("Participant disconnected: " + room.getParticipants().size());
    ...
    private Participant.Listener participantListener() {
        return new Participant.Listener() {
            ...
            public void onVideoTrackAdded(Participant participant, VideoTrack videoTrack) {
				...
				videoTrackAdded += 1;
				//videoStatusTextView.setText("Added videoTrackAdded " + videoTrackAdded);
            }
            ...
            public void onVideoTrackRemoved(Participant participant, VideoTrack videoTrack) {
				...
				videoTrackAdded -= 1;
				//videoStatusTextView.setText("Removed videoTrackAdded " + videoTrackAdded);
            }
